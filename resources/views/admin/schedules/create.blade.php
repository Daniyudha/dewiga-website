...
@section('title', 'Add Schedule - Admin Dewiga')

@section('content')
    {{-- Page Header --}}
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-heading font-bold text-gray-900">{{ __('Add Schedule') }}</h1>
            <p class="text-sm text-gray-500 mt-1">{{ __('Add new travel package schedule') }}</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.schedules.store') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="travel_package_id" id="travel_package_id" class="admin-input w-full @error('travel_package_id') border-red-500 @enderror" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="quota" id="quota"
                            ...
                            min="1" value="{{ old('quota', 100) }}" required>
                        @error('quota')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
