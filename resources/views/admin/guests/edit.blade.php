            ...
            <form method="POST" action="{{ route('admin.guests.update', $guest) }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                              class="admin-input w-full rounded-lg shadow-md border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 @enderror">{{ old('notes', $guest->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
