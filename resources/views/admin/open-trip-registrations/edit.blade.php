...
@section('title', 'Edit Open Trip Registration - Admin Dewiga')

@section('content')
    {{-- Page Header --}}
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-heading font-bold text-gray-900">{{ __('Edit Open Trip Registration') }}</h1>
            <p class="text-sm text-gray-500 mt-1">Update registration details</p>
        </div>
    ...
    <div class="admin-card">
        ...
        <div class="admin-card-body">
            <form method="post" action="{{ route('admin.open-trip-registrations.update', $openTripRegistration) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="admin-form-group">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $openTripRegistration->name) }}"
                    ...
                    <div class="admin-form-group">
                        ...
                        <input type="text" id="institution" name="institution" value="{{ old('institution', $openTripRegistration->institution) }}"
                    ...
                    <div class="admin-form-group">
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email', $openTripRegistration->email) }}"
                    ...
                    <div class="admin-form-group">
                        ...
                        <input type="text" id="number_phone" name="number_phone" value="{{ old('number_phone', $openTripRegistration->number_phone) }}"
                    ...
                    <div class="admin-form-group">
                        ...
                        <select id="schedule_id" name="schedule_id" class="admin-form-input @error('schedule_id') error @enderror" required>
                            ...
                            @foreach($schedules as $schedule)
                                ...
                                <option value="{{ $schedule->id }}" {{ old('schedule_id', $openTripRegistration->schedule_id) == $schedule->id ? 'selected' : '' }}>
                    ...
                    <div class="admin-form-group">
                        ...
                        <input type="number" id="people_count" name="people_count" value="{{ old('people_count', $openTripRegistration->people_count) }}"
                    ...
                    <div class="admin-form-group">
                        ...
                        <select id="status" name="status" class="admin-form-input @error('status') error @enderror" required>
                            @foreach(\App\Models\OpenTripRegistration::statuses() as $val => $label)
                                <option value="{{ $val }}" {{ old('status', $openTripRegistration->status) == $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('status')
                            <p class="admin-form-error">{{ $message }}</p>
                    ...
                    <div class="admin-form-group">
                        ...
                        <textarea id="notes" name="notes" rows="3" class="admin-form-input @error('notes') error @enderror"
                                  placeholder="Optional notes...">{{ old('notes', $openTripRegistration->notes) }}</textarea>
                        @error('notes')
                            <p class="admin-form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="admin-form-group md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3" class="admin-form-input @error('description') error @enderror"
                                  placeholder="Any special requests or notes...">{{ old('description', $openTripRegistration->description) }}</textarea>
                        @error('description')
                            <p class="admin-form-error">{{ $message }}</p>
                        @enderror
                    </div>
