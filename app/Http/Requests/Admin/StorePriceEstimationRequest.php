...
class StorePriceEstimationRequest extends FormRequest
{
    ...
    {
        $studentCount = (int) ($this->student_count ?? 0);
        $companionCount = (int) ($this->companion_count ?? 0);

        $this->merge([
            'service_participant_count' => $studentCount + $companionCount,
            'activity_participant_count' => $studentCount,
